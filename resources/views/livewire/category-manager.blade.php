<div>
    ...
    <div x-data="{ open: {{ $editingId ? 'true' : 'false' }} }" class="accordion" style="margin-bottom: 2rem;">
        <button @click="open = !open" type="button"
            ...
            <span>{{ $editingId ? 'Edit' : 'Add New' }} Category</span>
        ...
        <div x-show="open" x-transition
            ...
            <form wire:submit.prevent="save" style="max-width: 1000px; margin: 0 auto; padding: 1.5rem;">
                <div style="margin-bottom: 1rem;">
                    <label for="name" style="display: block; font-weight: 600; margin-bottom: 0.5rem;">Category
                        Name</label>
                    <input id="name" type="text" wire:model="name" placeholder="Category Name"
        ...
        @foreach ($categories as $category)
            <li
                ...
                <div style="display: flex; align-items: center;">
                    ...
                    @endif
                    <span style="font-weight: 600; color: #fff;">{{ ucwords($category->name) }}</span>
                    <span style="color: #9ca3af; margin-left: 0.75rem; font-size: 0.95em;">
                        <span
                            style="display: inline-block; padding: 0.15em 0.7em; border-radius: 12px; font-size: 0.85em; font-weight: 600; background: {{ $category->is_active ? '#22c55e' : '#6b7280' }}; color: #fff;">
